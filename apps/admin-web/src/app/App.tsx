...
function CrewPage({crew,error,reload}:{crew:CrewMember[];error:string;reload:()=>void}){const[search,setSearch]=useState(""),[filter,setFilter]=useState("전체 상태"),[page,setPage]=useState(1);const options=["전체 상태",...Array.from(new Set(crew.map((x)=>x.duty)))];const filtered=crew.filter((x)=>(filter==="전체 상태"||x.duty===filter)&&`${x.employeeNo}${x.name}${x.flight}${x.role}`.toLowerCase().includes(search.toLowerCase()));const safePage=Math.min(page,Math.max(1,Math.ceil(filtered.length/PAGE_SIZE)));const paged=filtered.slice((safePage-1)*PAGE_SIZE,safePage*PAGE_SIZE);const passportPending=crew.filter((x)=>/PENDING|WAITING|대기|미확인/i.test(x.passportStatus)).length;const operationPending=crew.filter((x)=>[x.baggageStatus,x.ticketStatus].some((value)=>/PENDING|WAITING|대기|미완료|미발권/i.test(value))).length;return <><div className="mini-kpis"><div><UsersRound size={19}/><span>조회 직원<strong>{crew.length}명</strong></span></div><div><ShieldCheck size={19}/><span>여권 확인 대기<strong>{passportPending}건</strong></span></div><div><Clock3 size={19}/><span>수하물·발권 대기<strong>{operationPending}건</strong></span></div></div><section className="panel page-panel"><SectionTitle title="직원 및 업무 처리 현황" subtitle="관리자는 현황만 조회하며 여권·수하물·발권 입력은 직원 페이지에서 처리합니다."/><DataError message={error}/><Toolbar value={search} setValue={(value)=>{setSearch(value);setPage(1)}} placeholder="직원번호, 이름, 항공편 검색" filter={filter} setFilter={(value)=>{setFilter(value);setPage(1)}} options={options} onRefresh={()=>{setPage(1);reload()}}/><div className="table-wrap"><table><thead><tr><th>직원번호</th><th>이름</th><th>직책</th><th>배정 항공편</th><th>근무 상태</th><th>여권 확인</th><th>수하물 처리</th><th>발권 처리</th></tr></thead><tbody>{paged.length?paged.map((x)=><tr key={x.employeeNo}><td className="strong-cell">{x.employeeNo}</td><td>{x.name}</td><td>{x.role}</td><td>{x.flight}</td><td><StatusPill value={x.duty}/></td><td><StatusPill value={x.passportStatus}/></td><td><StatusPill value={x.baggageStatus}/></td><td><StatusPill value={x.ticketStatus}/></td></tr>):<EmptyRow colSpan={8}/>}</tbody></table></div><PaginationControls page={safePage} total={filtered.length} onPageChange={setPage}/></section></>}
