...
```text
127.0.0.1 admin.youngkeke-air
```

이후 개발 주소는 `http://admin.youngkeke-air:5173`입니다. Apache 배포 후에는 포트 번호 없이 `http://admin.youngkeke-air`로 접속합니다.

## 실행

`start-admin-web.cmd`를 더블클릭하고 Windows 관리자 권한 요청을 승인하면 다음 작업을 한 번에 처리합니다.

1. `hosts` 파일에 `127.0.0.1 admin.youngkeke-air` 등록
2. DNS 캐시 갱신
3. 관리자 웹 개발 서버 실행
4. `http://admin.youngkeke-air:5173` 브라우저 열기
